...
func TestSlashingWithConsensusWeight(t *testing.T) {
	var err error
	app, ctx := createTestContext(t)
	startTime := time.Now()
	ctx = ctx.WithBlockTime(startTime).WithBlockHeight(1)
	params := types.DefaultParams()
	receiver := test_helpers.AddTestAddrsIncremental(app, ctx, 1, nil)
	params.SlashReceiver = receiver[0].String()
		...
		Assets: []types.AllianceAsset{
			{
				...
				ConsensusWeight: sdk.MustNewDecFromStr("0.1"),
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		...
		sdk.NewCoin(AllianceDenom, sdk.NewInt(200_000_000)),
		sdk.NewCoin(AllianceDenomTwo, sdk.NewInt(200_000_000)),
	))
	...
	_, err = app.StakingKeeper.Delegate(ctx, user1, sdk.NewInt(100_000_000), stakingtypes.Unspecified, *val1.Validator, true)
	require.NoError(t, err)
	...
	_, err = app.StakingKeeper.Delegate(ctx, user2, sdk.NewInt(100_000_000), stakingtypes.Unspecified, *val2.Validator, true)
	require.NoError(t, err)
	...
	// require.NoError(t, err)
	// require.Equal(t, sdk.NewInt(200_000_000), app.StakingKeeper.TotalBondedTokens(ctx))
...
func TestSlashingEvent(t *testing.T) {
	...
	// Accounts
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(20_000_000)),
	...
	// Users add delegations
	_, err = app.StakingKeeper.Delegate(ctx, user1, sdk.NewInt(10_000_000), stakingtypes.Unspecified, *val1.Validator, true)
	require.NoError(t, err)
